...
from tool import set_current_thread_id


app = FastAPI()

# React frontend ko FastAPI API access karne dena
app.add_middleware(
    CORSMiddleware,
        ...
        "conversation":[
            {
                ...
                "upadated_at":item.updated_at.isoformat()
            }
            for item in items
        ]
    }
...
async def upload_document(file:UploadFile = File(...),thread_id:str = Form(...)):
    try:
        ...
        file_path = f"upload/{file_id}_{safe_filename}"
        ...
        result = add_document_To_rag(file_path=file_path,thread_id=thread_id)
